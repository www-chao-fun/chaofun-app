import 'package:flutter_chaofan/database/database.dart';
...
class ComHelper {
  static final ComHelper _instance = ComHelper.internal();
  factory ComHelper() => _instance;
...
//插入
  Future<int> saveForumItem(item) async {
    var dbClient = await db;
    int res = await dbClient.insert("table_forum", item);
  ...
  Future<List> hasData() async {
    ...
    if (await isTableExits(dbs, 'table_forum')) {
      var result = await dbs.rawQuery("SELECT * FROM table_forum ");
      return result.toList();
    } else {
      return [];
    }
