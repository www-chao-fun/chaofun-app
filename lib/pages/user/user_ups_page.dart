...
class UserUpsPage extends StatefulWidget {
  final Map arguments;
  UserUpsPage({Key key, this.arguments}) : super(key: key);
  _UserUpsPageState createState() => _UserUpsPageState();
}

class _UserUpsPageState extends State<UserUpsPage> {
  String forumId;
  Map forumData;
  List<Map> pageData = [];
  ...
  void initState() {
    ...
    dataFuture = homeService.myListUpvotes(params, (response) {
      var data = response;
  ...
  void getDatas(type) async {
    ...
    var data = await HttpUtil().get(Api.myListUpvotes, parameters: params);
